import { Controller, Get, Post, Body, Patch, Param, Delete, Query, ParseUUIDPipe, ParseIntPipe, HttpStatus, } from '@nestjs/common';
import { ApiTags, ApiOperation, ApiResponse, ApiParam, ApiQuery, ApiBearerAuth, ApiBody } from '@nestjs/swagger';
import { BlockedUsersService } from './blocked-users.service';
import { BlockUserDto } from './block-user.dto';
import { BlockedUser } from './blocked-user.entity';

@ApiTags('User - Blocked Users')
@ApiBearerAuth()
...
export class BlockedUsersController {
  ...
  @ApiParam({ name: 'userId', type: 'string', format: 'uuid', description: 'User ID who is blocking', })
  @ApiResponse({ status: HttpStatus.CREATED, description: 'User blocked successfully', type: BlockedUser, })
  @ApiResponse({ status: HttpStatus.CONFLICT, description: 'User is already blocked', })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'User to block not found', })
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Cannot block yourself', })
  async blockUser(@Param('userId', ParseUUIDPipe) userId: string, @Body() blockUserDto: BlockUserDto): Promise<BlockedUser> {
  ...
  @ApiParam({ name: 'userId', type: 'string', format: 'uuid', description: 'User ID' })
  @ApiResponse({ status: HttpStatus.OK, description: 'Blocked users count retrieved successfully', schema: { type: 'number' } })
  ...
  @ApiQuery({ name: 'page', required: false, type: Number, description: 'Page number (default: 1)' })
  @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Items per page (default: 10)' })
  @ApiResponse({ status: HttpStatus.OK, description: 'Users blocking this user retrieved successfully' })
  ...
  @ApiParam({ name: 'userId', type: 'string', format: 'uuid', description: 'User ID', })
  @ApiParam({ name: 'targetUserId', type: 'string', format: 'uuid', description: 'Target user ID to check', })
  @ApiResponse({ status: HttpStatus.OK, description: 'Block status retrieved successfully', schema: { type: 'boolean' }, })
  async isUserBlocked(
    @Param('userId', ParseUUIDPipe) userId: string,
  ...
  @ApiParam({ name: 'userId', type: 'string', format: 'uuid', description: 'User ID' })
  @ApiParam({ name: 'blockedUserId', type: 'string', format: 'uuid', description: 'Blocked user ID' })
  @ApiResponse({ status: HttpStatus.OK, description: 'Blocked user retrieved successfully', type: BlockedUser })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Blocked user not found' })
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'Block reason updated successfully', type: BlockedUser })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Blocked user not found' })
